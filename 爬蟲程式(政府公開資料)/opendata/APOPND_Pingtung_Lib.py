...
Log_path = os.path.join(path,"log","APOPND_Pingtung_Library.log")

logging.basicConfig(level=logging.INFO,
					format='[%(asctime)-4s] %(message)s',
					datefmt='%Y-%m-%d %H:%M:%S',
					handlers = [logging.FileHandler(Log_path, 'w', 'utf-8'),])
class APOPND_Pingtung_Library(): #主要抓的地方
	def __init__(self):
		self.GetCSVurl()
		self.APOPND_Pingtung_Library_crawler()

	def GetCSVurl(self):
		global URL_LIST

		url = "https://data.gov.tw/dataset/86192"
		...
		for csv_url in data_url:
			if '.csv' in csv_url.get('href') and csv_url.get('href').replace('https', 'http') not in URL_LIST:
				URL_LIST.append(csv_url.get('href').replace('https', 'http'))

	def APOPND_Pingtung_Library_crawler(self):
		logging.info("屏東圖書館資料")
		...
		for url in URL_LIST:
			...
			if(res.status_code == 200):
				Filename = 'APOPND_Pingtung_Library.csv' #這裡要改
