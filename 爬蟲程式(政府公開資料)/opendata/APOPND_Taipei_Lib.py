...
Log_path = os.path.join(path,"log","APOPND_Taipei_Library.log")

logging.basicConfig(level=logging.INFO,
					format='[%(asctime)-4s] %(message)s',
					datefmt='%Y-%m-%d %H:%M:%S',
					handlers = [logging.FileHandler(Log_path, 'w', 'utf-8'),])
class APOPND_Taipei_Library(): #主要抓的地方
	def __init__(self):
		self.GetCSVurl()
		self.APOPND_Taipei_Library_crawler()

	def GetCSVurl(self):
		url = "https://data.gov.tw/dataset/61850"
	...
	def APOPND_Taipei_Library_crawler(self):
		logging.info("台北圖書館資料")
		...
		for url in URL_LIST:
			...
			if(res.status_code == 200):
				Filename = 'APOPND_Taipei_Library.csv' #這裡要改
