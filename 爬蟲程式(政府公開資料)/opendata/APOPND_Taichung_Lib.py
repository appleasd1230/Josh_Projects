...
Log_path = os.path.join(path,"log","APOPND_Taichung_Library.log")
...
class APOPND_Taichung_Library(): #主要抓的地方
	def __init__(self):
		self.GetCSVurl()
		self.APOPND_Taichung_Library_crawler()

	def GetCSVurl(self):
		global URL_LIST

		url = "https://data.gov.tw/dataset/83865"
		...
		data_url = "http://datacenter.taichung.gov.tw/swagger/OpenData/e3925fec-6459-4486-be89-d764b5ee8050"

		URL_LIST.append(data_url.replace('https', 'http'))

	def APOPND_Taichung_Library_crawler(self):
		logging.info("台中圖書館資料")
		...
		for url in URL_LIST:
			...
			if(res.status_code == 200):
				Filename = 'APOPND_Taichung_Library.csv' #這裡要改
