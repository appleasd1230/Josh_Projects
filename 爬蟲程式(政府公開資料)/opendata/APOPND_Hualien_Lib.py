...
Log_path = os.path.join(path,"log","APOPND_Hualien_Library.log")

logging.basicConfig(level=logging.INFO,
					format='[%(asctime)-4s] %(message)s',
					datefmt='%Y-%m-%d %H:%M:%S',
					handlers = [logging.FileHandler(Log_path, 'w', 'utf-8'),])
class APOPND_Hualien_Library(): #主要抓的地方
	def __init__(self):
		self.GetCSVurl()
		self.APOPND_Hualien_Library_crawler()

	def GetCSVurl(self):
		global URL_LIST

		url = "https://data.gov.tw/dataset/72480"
	...
	def APOPND_Hualien_Library_crawler(self):
		logging.info("花蓮圖書館資料")
		...
		for url in URL_LIST:
			...
			if(res.status_code == 200):
				Filename = 'APOPND_Hualien_Library.csv' #這裡要改
