...
Log_path = os.path.join(path,"log","APOPND_Taoyuan_Library.log")

logging.basicConfig(level=logging.INFO,
					format='[%(asctime)-4s] %(message)s',
					datefmt='%Y-%m-%d %H:%M:%S',
					handlers = [logging.FileHandler(Log_path, 'w', 'utf-8'),])
class APOPND_Taoyuan_Library(): #主要抓的地方
	def __init__(self):
		self.GetCSVurl()
		self.APOPND_Taoyuan_Library_crawler()

	def GetCSVurl(self):
		global URL_LIST

		url = "https://data.gov.tw/dataset/30908"
		...
		data_url = 'https://quality.data.gov.tw/dq_download_csv.php?nid=30908&md5_url=bb2cf159534e9670ccb69d8114c909d0'

		URL_LIST.append(data_url.replace('https', 'http'))

	def APOPND_Taoyuan_Library_crawler(self):
		logging.info("桃園圖書館資料")
		...
		for url in URL_LIST:
			...
			if(res.status_code == 200):
				Filename = 'APOPND_Taoyuan_Library.csv' #這裡要改
